...
# nohup python3 Perdidas_doble_corr.py > Perdidas_doble_corr.out &
...
# Cartella principale
n_exp = n_experiments
base_dir = f"Experiment_con_doble_correciones_{n_exp}"
os.makedirs(base_dir, exist_ok=True)

# Sottocartella per questa percentuale
exp_subdir = os.path.join(base_dir, f"exp_con_dob_cor_{perc100}_{dim1}_{n_exp}")
os.makedirs(exp_subdir, exist_ok=True)

# Sottocartella dei grafici 
grafici_dir = os.path.join(exp_subdir, f"grafici_dob_cor_{n_exp}")
os.makedirs(grafici_dir, exist_ok=True)


# File CSV con perc100 nel nome
csv_completo = os.path.join(exp_subdir, f"valori_completi_dob_cor_{perc100}_{dim1}_{n_exp}.csv")
with open(csv_completo, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Experiment", "Iteration", "l_c", "r_c", "l_n", "r_n", "V", "w"])

csv_finale = os.path.join(exp_subdir, f"valori_finali_dob_cor_{perc100}_{dim1}_{n_exp}.csv")
with open(csv_finale, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Experiment", "l_c", "r_c", "l_n", "r_n", "V", "w"])

csv_best = os.path.join(exp_subdir, f"best_dob_cor_{perc100}_{dim1}_{n_exp}.csv")
...
class Environment:
    ...
    def compute_w(self, s):
        l_c, r_c, l_n, r_n = s
        rr = r_n/r_c
        dl1 = 0.82 * (1-1.35*rr+0.31*rr**3)*r_n
        dl2 = 0.82 * r_n
        ...
        neck = Neck(l = l_n, dl = dl2, r = r_n)
        cavity = Cavity(l = l_c, dl = dl1, r = r_c)
        
        w, _ = solucion_con_perdidas_correciones(neck, cavity, air, geom, 1, 2, tol, maxit)
        return w
...
for j in tqdm(range(n_experiments)):
    ...
    if j % 5 == 0:
        ...
        plt.title(f"Progress of variables for experiment {j}/{n_experiments}", fontsize=16)
        plt.legend(fontsize=17)
        plt.grid(True)
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
        plt.tight_layout()
        plt.savefig(os.path.join(grafici_dir, f"variabili_esperimento_{j}_corr_{perc100}_{dim1}_{n_exp}.png"), dpi=300)
        #plt.show()
        plt.close()
...
print('Minimum volume: ', V_min, ', Minimum angular frequency: ', w_min, ', Minimum frequency: ', f_min)
print('Optimal volume: ', V_opt, ', Angular frequency at optimum: ', w_opt, ', Frequency at optimum: ', f_opt)
...
plt.title("Distribution of results in the V-ω plane (colored by experiment)")
plt.colorbar(scatter, label="Experiment Number")
plt.grid(True)

# 
perc_text = f"Randomization percentage: {perc * 100:.1f}%"
info_text = (
    f"$\omega_{{min}}$ = {w_min:.3f}, $f_{{min}}$ = {f_min:.3f} Hz, $V_{{min}}$ = {V_min:.2e}\n"
    f"$\omega_{{opt}}$ = {w_opt:.3f}, $f_{{opt}}$ = {f_opt:.3f} Hz, $V_{{opt}}$ = {V_opt:.2e}\n"
    f"{perc_text}"
)
plt.gcf().text(0.5, -0.05, info_text, fontsize=10, ha='center', va='top', bbox=dict(facecolor='white', alpha=0.8))

plt.tight_layout()
plt.savefig(os.path.join(grafici_dir, f"results_V_w_{perc100}_{dim1}_{n_exp}.png"), dpi=300, bbox_inches='tight')
plt.show()
...
# 
info_text = rf"$n_{{\mathrm{{neu}}}} = {dim1},\quad p\% = {perc100}$"
plt.gcf().text(0.5, 0.1, info_text, fontsize=10, ha='center', va='bottom',
               bbox=dict(facecolor='white', alpha=0.7))

# Salva immagine
pareto_plot_path = os.path.join(grafici_dir, f"pareto_front_{perc100}_{dim1}_{n_exp}.png")
