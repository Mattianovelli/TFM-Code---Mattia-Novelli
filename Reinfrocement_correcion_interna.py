...
# Cartella principale
n_exp = n_experiments
base_dir = f"Experiment_con_correciones_{n_exp}"
os.makedirs(base_dir, exist_ok=True)

# Sottocartella per questa percentuale
exp_subdir = os.path.join(base_dir, f"exp_con_cor_{perc100}_{dim1}_{n_exp}")
os.makedirs(exp_subdir, exist_ok=True)

# Sottocartella dei grafici 
grafici_dir = os.path.join(exp_subdir, "grafici_cor_{n_exp}")
os.makedirs(grafici_dir, exist_ok=True)


# File CSV con perc100 nel nome
csv_completo = os.path.join(exp_subdir, f"valori_completi_cor_{perc100}_{dim1}_{n_exp}.csv")
with open(csv_completo, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Experiment", "Iteration", "l_c", "r_c", "l_n", "r_n", "V", "w"])

csv_finale = os.path.join(exp_subdir, f"valori_finali_cor_{perc100}_{dim1}_{n_exp}.csv")
with open(csv_finale, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Experiment", "l_c", "r_c", "l_n", "r_n", "V", "w"])

csv_best = os.path.join(exp_subdir, f"best_cor_{perc100}_{dim1}_{n_exp}.csv")
...
class Environment:
    ...
    def compute_w(self, s):
        ...
        neck = Neck(l = l_n, dl = 0, r = r_n)
        cavity = Cavity(l = l_c, dl = dl1, r = r_c)
        
        w, _ = solucion_con_perdidas_correciones(neck, cavity, air, geom, tol, maxit)
        return w
    ...
    def step(self, action_idx):
        ...
        new_state[var_idx] = np.clip(new_state[var_idx], self.lim_min[var_idx], self.lim_max[var_idx]) 
