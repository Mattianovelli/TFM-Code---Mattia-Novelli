...
# Cartella principale
base_dir = f"Experiment_sin_correciones_{n_exp}"
os.makedirs(base_dir, exist_ok=True)

# Sottocartella per questa percentuale
exp_subdir = os.path.join(base_dir, f"exp_sin_cor_{perc100}_{dim1}_{n_exp}")
os.makedirs(exp_subdir, exist_ok=True)

# Sottocartella dei grafici 
grafici_dir = os.path.join(exp_subdir, "grafici")
os.makedirs(grafici_dir, exist_ok=True)


# File CSV con perc100 nel nome
csv_completo = os.path.join(exp_subdir, f"valori_completi_{perc100}_{dim1}_{n_exp}.csv")
with open(csv_completo, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Experiment", "Iteration", "l_c", "r_c", "l_n", "r_n", "V", "w"])

csv_finale = os.path.join(exp_subdir, f"valori_finali_{perc100}_{dim1}_{n_exp}.csv")
with open(csv_finale, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Experiment", "l_c", "r_c", "l_n", "r_n", "V", "w"])

csv_best = os.path.join(exp_subdir, f"best_{perc100}_{dim1}_{n_exp}.csv")
...
class Environment:
    ...
    def compute_w(self, s):
        ...
        cavity = Cavity(l = l_c, dl = 0, r = r_c)
        
        w, _ = solucion_con_perdidas(neck, cavity, air, geom, tol, maxit)
        return w
...
for j in tqdm(range(n_experiments)):
    ...
    if j % 5 == 0:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(grafici_dir, f"variabili_esperimento_{j}_no_corr_{perc100}_{dim1}_{n_exp}.png"), dpi=300)
        #plt.show()
        plt.close()
