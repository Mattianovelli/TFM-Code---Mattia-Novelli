# Scelta Pareto - confronto tra 5 curve
import numpy as np
...
plt.xlabel('V')
plt.ylabel('w')
plt.title('Confronto fra 5 fronti di Pareto (V < $1\\cdot10^{-4}$, w < 500)')
plt.legend()
plt.grid(True)

# Limiti solo superiori
plt.xlim(right=1e-4)
plt.ylim(top=500)
...
plt.savefig(os.path.join(folder_name, "pareto_confronto_5_fronti.png"))
