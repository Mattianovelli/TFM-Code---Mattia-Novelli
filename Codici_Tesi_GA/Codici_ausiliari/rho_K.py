...
from Codici_ausiliari.G import G
from Codici_ausiliari.sums import sum_approximations
from Codici_ausiliari.air_features import air_features
air = air_features()

def rho_K(oggetto, geom):
    ...
    if geom == 1:
        ...
        a = oggetto.a
        b = oggetto.b
        sum_approx_rho, sum_approx_K = sum_approximations(a, b)
        def rho(w):
        ...
        def rho(w):
            ...
            # Approssimazione per rG --> inf
            if not np.any(np.isfinite(val)):
                #print('Approssimazione: w = ', w, ', rG = ', rG)
                return rho_0
            return val
